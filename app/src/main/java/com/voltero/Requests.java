...
public class Requests
{
    ...
    public static void RunnableRequest(Runnable activity, String filename, ContentValues params, RequestHandler handler)
    {
        HttpUrl.Builder URL = Objects.requireNonNull(HttpUrl.parse(URL_BASE + filename + ".php")).newBuilder();
        ...
        client.newCall(req).enqueue(new Callback()
        {
            ...
            public void onFailure(@NotNull Call call, @NotNull IOException e)
            {
                Toast.makeText((Context) activity, ERROR_MSG, Toast.LENGTH_SHORT).show(); // Show error message
            }
